export function upconvertFromANSI(text: string): string {
    ...
    return text.replace(/./g, (char) => {
        ...
        if (uniToCp1252[code]) {
            return String.fromCharCode(uniToCp1252[code] + 0xF000);
        } else if (code < 256) {
...
console.log(upconvertFromANSI("n<Š]Å¡"));
import { anu7ToUnicode } from './src/utils/anu7Converter.ts';
console.log(anu7ToUnicode(upconvertFromANSI("n<Š]Å¡")));
