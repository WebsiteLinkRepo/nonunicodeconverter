...
// Replace scriptSelectEl event listener logic
const scriptSelectLogicRegex = /const scriptSelectEl = document\.getElementById\('script-select'\) as HTMLSelectElement;\s*if \(scriptSelectEl\) \{\s*scriptSelectEl\.addEventListener\('change', \(\) => \{\s*processConversion\(false\);\s*\}\);\s*\}/;
...
content = content.replace(scriptSelectLogicRegex, dynamicScriptLogic);

fs.writeFileSync('src/components/TextConverter.astro', content);
console.log("Patched TextConverter.astro successfully");
