...
"""
content = re.sub(r'(  fonts: {)', interface_addition.lstrip('\n') + r'\1', content)

# Add to each language
for lang, obj in tooltips.items():
    # Find language block: lang: {
    pattern = rf"({lang}:\s*{{.*?)(    fonts: {{)"
    replacement_str = ""
    for k, v in obj.items():
        replacement_str += f'    {k}: "{v}",\n'
    
    content = re.sub(pattern, r'\1' + replacement_str + r'\2', content, flags=re.DOTALL)
