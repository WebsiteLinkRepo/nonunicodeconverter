...
function compareLines(actual: string, expected: string) {
    ...
    if (aLine !== eLine) {
        ...
        for(let j=0; j<Math.max(aChars.length, eChars.length); j++) {
            if(aChars[j] !== eChars[j]) {
                const aVal = aChars[j] ? aChars[j].charCodeAt(0).toString(16).padStart(4, '0') : 'none';
                const eVal = eChars[j] ? eChars[j].charCodeAt(0).toString(16).padStart(4, '0') : 'none';
                console.log(`Diff at ${j}: '${aChars[j]}' (\\u${aVal}) vs E: '${eChars[j]}' (\\u${eVal})`);
            }
        }
    }
  }
