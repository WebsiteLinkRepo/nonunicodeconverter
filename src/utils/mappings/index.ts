...
export const AVAILABLE_FONTS: FontOption[] = [
  ...
  },
  {
    ...
    family: 'Hari',
    script: 'gujarati',
    description: 'Legacy font layout for Gujarati',
    fallbackFontFamily: "'Hari', sans-serif"
  }
];
...
export function getMapping(encoding: FontEncoding, reverse: boolean = false) {
  ...
  if (encoding === 'anu6') {
    ...
    script = 'tamil';
  } else if (encoding === 'hari') {
    mappingList = HARI_GUJARATI_MAPPINGS;
    script = 'gujarati';
  }
  ...
  const blockOffsets: Record<string, number> = {
    ...
    gujarati: 0x0180     // Gujarati (0x0A80) -> Telugu (0x0C00) - Just keeping same struct though we probably just use raw mapping
  };

  const offset = blockOffsets[script] || 0;
  let processedMapping = mappingList;

  if (offset !== 0 && encoding !== 'hari') { // Bypass dynamic shift for explicitly mapped hari font
