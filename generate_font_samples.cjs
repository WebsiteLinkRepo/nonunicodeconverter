...
const fonts = fs.readdirSync(fontsDir).filter(f => f.endsWith('.TTF?v=2') || f.endsWith('.ttf')).slice(0, 50);
...
fs.writeFileSync(path.join(__dirname, 'public/font_samples.html'), html);
console.log('Created font_samples.html');
